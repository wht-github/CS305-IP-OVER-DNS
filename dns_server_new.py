    ...
    def run(self):
        ...
        while True:
            ...
            if self._socket in w and not self.data_queue.empty():
                ...
                print(str(coder.b64encode(data_to_socket), encoding='ascii'))
                print(len(str(coder.b64encode(data_to_socket), encoding='ascii')))
                # ip_response = str(coder.b64encode(
                #     data_to_socket), encoding='ascii')
                # # if (len(ip_response) > 255):
                # for i in range(0, len(ip_response), 200):

                #     response.answer.append(dns.rrset.from_text(
                #         query_msg.question[0].name, 30000, 1, 'TXT',ip_response[i:i+200]))
                response.answer.append(dns.rrset.from_text(
                    query_msg.question[0].name, 30000, 1, 'TXT', str(coder.b64encode(data_to_socket), encoding='ascii')))
                print(data_to_socket)
                # response.answer.append(dns.rrset.from_text(
                #     query_msg.question[0].name, 30000, 1, 'TXT', 'hello'))
